...
A join operation $\sqcup$ defines a semi-lattice: it induces a partial order $\le_\sqcap$ over its domain \todo{tu trzeba wiecej komentarza, np. czemu taki porzadek istniej i jak jest zdefiniowany (a najlepiej konstruktywnie go zdefiniowac i pokazac ze to porzadek)}, such that the result of the operation for any two elements is the least upper bound of those elements with respect to $\le_\sqcap$.
...
\end{thm}
\begin{prof}
$G_P$ is monotone with respect to $\sqsubseteq_\sigma$ on all databases over $\sigma$. \todo {czemu?} Since it is assumed that $T_{P'}$ is monotone with respect to $\sqsubseteq_\sigma$ on databases after aggregation, $T_P = G_P \circ T_{P'}$ is also monotone with respect to $\sqsubseteq_\sigma$, as a composition of two monotone functions. 
...
\todo{tu jest coś nie tak, brakuje kropki i następne zdanie nie jest zdaniem dale się pogubiłem - widzę, że zaczynasz rozpatrywać przypadki, ale nigdzie nie napisałeś że zaczynasz - zazwyczaj pisze się, że możliwe są (1), ..., (2), ... (3) ... żeby było jasne że to wszystkie możliwści i potem się kolejne rozważa sprawdź to solidnie!}
