...
Syntactically, we require that each \emph{idb} relation is declared at the top of the program as on the example below.
...
If $R$ does have an aggregated column, then by Lemma \ref{lem:preorder} it is a pre-order over $\ps({\bf dom}^k)~\supseteq~Z_R$, so it only remains to be shown that $\sqsubseteq_R$ is antisimmetric.
...
Let $P$ be a \datalogra program, with $w$ \emph{idb} relations $R_1, R_2, \dots, R_w$ of arities $k_1, k_2, \dots, k_w$ respectively. $P$ has the form of:
\begin{figure}[h!]
...
The subgoals in the rule bodies may refer to any relation of an arbitrary set of \emph{edb} relations $edb(P)$, which are constant during the evaluation, and to any of the \emph{idb} relations, $R_1, \dots, R_w$.
...
Another important building block in \datalogra semantics is an aggregation operator on database instances. Intuitively, it applies of the corresponding aggregation operator $g_R$ to each relation instance $I$ of relation $R$ in a database instance.
...
\todo{Czy ten dowód to nie jest zbyt mocne powtórzenie \ref{t:datalogfixpointsem}?}
\end{prof}
